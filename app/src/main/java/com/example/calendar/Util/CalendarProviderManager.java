...
public class CalendarProviderManager {
    ...
    private static long checkCalendarAccount(Context context) {
        try (@SuppressLint("MissingPermission") Cursor cursor = context.getContentResolver().query(CalendarContract.Calendars.CONTENT_URI,
                null, null, null, null)) {
            // 不存在日历账户
            if (null == cursor) {
                return -1;
    ...
    private static String getFullRRuleForRRule(String rRule, long beginTime, long endTime) {
        ...
        switch (rRule) {
            ...
            case RRuleConstant.REPEAT_WEEKLY_BY_SU:
                return builder.append(rRule).append(TimerUtil.getFinalRRuleMode(endTime)).toString();
            case RRuleConstant.REPEAT_CYCLE_WEEKLY:
                return builder.append(rRule).append(TimerUtil.getWeekForDate(beginTime)).append("; UNTIL = ")
                        .append(TimerUtil.getFinalRRuleMode(endTime)).toString();
            case RRuleConstant.REPEAT_CYCLE_MONTHLY:
                return builder.append(rRule).append(TimerUtil.getDayOfMonth(beginTime))
                        .append("; UNTIL = ").append(TimerUtil.getFinalRRuleMode(endTime)).toString();
            default:
                return rRule;
        }
    }
